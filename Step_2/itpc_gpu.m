function [time_axis, log_frequencies, itpc] = itpc_gpu(save_path, inTitle, EEG, highpass, lowpass, plotMe)
...
% This function uses the GPU and expects there to be an appropriate GPU to use, otherwuse use itpc_snr.m
...
n_elecs = size(EEG.data, 1); % Number of electrodes
fs = EEG.srate; % Sampling frequency

% Preallocate for ITPC computation (on GPU)
n_freq = length(frequencies);
n_time = size(EEG.data, 2);
phase_data = gpuArray(nan(n_elecs, n_trials, n_freq, n_time));

% Move EEG data to GPU
EEG_data_gpu = gpuArray(EEG.data);

% Clean up
clear EEG

% Loop through trials (parallelized on GPU)
for trial = 1:n_trials
    for elec = 1:n_elecs
        % Get trial data (from GPU)
        trial_data = squeeze(EEG_data_gpu(elec, :, trial)); % Change '1' to desired channel index
        ...
        for f = 1:n_freq
            ...
            wavelet = gpuArray(exp(2 * 1i * pi * freq * t) .* exp(-t.^2 / (2 * sigma_t^2)));

            % Convolve wavelet with trial data (GPU operation)
...
% Compute ITPC (on GPU)
itpc_gpu = abs(mean(exp(1i * phase_data), 2)); % Average across trials

% Gather data back from GPU
itpc = gather(itpc_gpu);
